...
void edsign_sec_to_pub(uint8_t *pub, uint8_t *prv, const uint8_t *secret);
...
void omemoDriverEdSignMod(omemoCurveSignature signature, omemoKey pub, omemoKey secret, uint8_t *message, size_t len) {
	uint8_t e[FPRIME_SIZE];
	uint8_t s[FPRIME_SIZE];
	uint8_t k[FPRIME_SIZE];
	uint8_t z[FPRIME_SIZE];
...
int omemoDriverX25519(omemoKey out, omemoKey prv, omemoKey pub) {
  c25519_smult(out, pub, prv);
  return !f25519_eq(out, f25519_zero) ? 0 : OMEMO_ECORRUPT;
}
